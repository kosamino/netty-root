...
public class DelimiterNettyServerHandler extends ChannelInboundHandlerAdapter {

    private AtomicInteger readCount =new AtomicInteger(0);
    private AtomicInteger completeCout =new AtomicInteger(0);

    public DelimiterNettyServerHandler(){
    ...
    throws  Exception{
        ...
        ReferenceCountUtil.release(byteBuf);//0

        ReferenceCountUtil.touch(byteBuf);//

        ByteBuf response= Unpooled.copiedBuffer(("hello netty client"
        + DelimiterNettyClientHandler.SYMBOL).getBytes());
